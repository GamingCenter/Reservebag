...
import "./interface/TeamPerfitForwarderInterface.sol";
import "./interface/DRSCoinInterface.sol";

import "./library/SafeMath.sol";
import "./library/NameFilter.sol";
import "./library/DRSDatasets.sol";
import "./DRSEvents.sol";
...
contract ReserveBag is DRSEvents {
    ...
    uint256 constant private initKeyPrice = 50000000;
    ...
    //uint256 constant private rndMax_ = 600 seconds;                // max length a round timer can be
    ...
    uint256 public keyPriceIncreaseRatio = 15;
    uint256 public genRatio = 700;

    uint256 public drsCoinDividendRatio =25;
    uint256 public teamPerfitRatio = 0;

    uint256 public ethMintDRSCoinRate = 1;
    uint256 public ethMintRate = 1;
    ...
    modifier isWithinLimits(uint256 _eth) {
        require(_eth >= 100000, "pocket lint: not a valid currency");
        require(_eth <= 1000 * (10**14), "no Sun, no");
        _;
    }
    ...
    function buyKeyXaddr(address _affCode)
        isActivated()
        isHuman()
        isWithinLimits(msg.value)
        public
        payable
    {
        ...
        // if affiliate code was given    
            ...
            if (_affID != plyr_[_pID].laff && plyr_[_pID].laff == 0)
            {
        ...
        public
    {
        ...
        // reload core
        reLoadCore(_pID,plyr_[_pID].laff, _eth, _eventData_);
    }
     ...
     */
    function buyCore(uint256 _pID,uint256 _affID, DRSDatasets.EventReturns memory _eventData_) private
    {
        uint256 _rID = rID_;
        ...
        if(_now >= round_[_rID].strt.add(rndGap_) && (_now <= round_[_rID].end || round_[_rID].plyr == 0)) {
            // call core
            core(_rID, _pID,_affID, msg.value, _eventData_);
     ...
     * incoming eth depending on if we are in an active round or not
     */
    function reLoadCore(uint256 _pID,uint256 _affID, uint256 _eth, DRSDatasets.EventReturns memory _eventData_) private
    {
        uint256 _rID = rID_;
        ...
        if(_now > round_[_rID].strt.add(rndGap_) && (_now <= round_[_rID].end || round_[_rID].plyr == 0)) {
            ...
            // call core
            core(_rID, _pID,_affID, _eth, _eventData_);
     ...
     */
    function core(uint256 _rID, uint256 _pID,uint256 _affID, uint256 _eth, DRSDatasets.EventReturns memory _eventData_) private
    {
        if(_eth < keyPrice) {
            plyr_[_pID].gen = plyr_[_pID].gen.add(_eth);
            emit onBuyKeyFailure(_rID, _pID, msg.sender,_eth, keyBought, now);
            return;
        }
        ...
        // distribute eth
        uint256 _ethExt = distributeExternal(_ethUsed,_affID,_pID);
        ...
        drsCoin.mint(plyr_[_pID].addr, _ethUsed.mul(ethMintRate).div(5000));
    ...
    function mintDRSCoin() private {
        ...
        for(uint256 i = startIndex; (i + 1) % (rewardInternal + 1) != endIndex; i = (i + 1) % (rewardInternal + 1)) {
            drsCoin.mint(buyinfos[i].addr, buyinfos[i].keyPrice.mul(ethMintDRSCoinRate).div(500));
        }
    }
     ...
     */
    function distributeExternal(uint256 _eth,uint256 _affID,uint256 _pID) private returns(uint256)
    {
        // pay 7.5% out to community rewards
        uint256 _com = _eth.mul(75).div(1000);
        uint256 _aff =0;
        ...
        {
            _aff =  _eth/100;
            _com = _eth.mul(65).div(1000);
            plyr_[_affID].addr.transfer(_aff);
            plyr_[_affID].aff = plyr_[_affID].aff.add(_aff);
            emit DRSEvents.onAffiliatePayout(_affID, plyr_[_affID].addr, plyr_[_affID].name, _pID, _aff, now);
        } 

        depositTeamPerfit(_com);

        return _com.add(_aff);
    }
