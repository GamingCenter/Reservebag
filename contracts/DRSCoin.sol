...
contract DRSCoin {
  ...
  uint256 public constant tokenExchangeLeastRate = 1; // 10 tokens per 1 ETH at least
  uint256 public constant tokenReduceValue = 70000000;
  uint256 public constant supplyForMintValue = 700000000;
  uint256 public constant supplyForOtherValue = 300000000;
  uint256 public constant coinReduceRate = 90;
  
  //uint256 constant private proposingPeriod = 1 days;
  ...
  uint256 public totalSupplyForMint = 0;
  uint256 public totalSupplyForOther = 0;
  ...
  uint256 public supplyForMint;
  uint256 public supplyForOther;
  ...
  mapping(address => bool) public allowedGameAddress;
  ...
  constructor() public
  {
    ...
    supplyForMint = supplyForMintValue.mul(uint256(10) ** uint256(decimals));
    supplyForOther= supplyForOtherValue.mul(uint256(10) ** uint256(decimals));
  ...
  function mint(address _to, uint256 _eth) external onlyAllowedGameAddress {
    ...
    require(totalSupplyForMint + _amount < supplyForMint);
    ...
    totalSupplyForMint = _amount.add(totalSupplyForMint);
...
function mintOther(address _to,uint256 _amount) external onlyAllowedGameAddress {
   
    require(_amount > 0);
    require(totalSupply + _amount > totalSupply);
    require(totalSupplyForOther + _amount < supplyForOther);
    require(balanceOf[_to] + _amount > balanceOf[_to]); // Check for overflows
    ...
    totalSupply = _amount.add(totalSupply); // Update total supply
    totalSupplyForOther = _amount.add(totalSupplyForOther);
    balanceOf[_to] = _amount.add(balanceOf[_to]); // Set minted coins to target
